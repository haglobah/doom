...
(cl-defun bah/get-workspace (name workspaces)
  ...
  (dolist (name+dir+file workspaces)
    (if (string-equal name (cl-first name+dir+file))
        (cl-return-from bah/get-workspace name+dir+file)
      nil)))

(defun bah/setup-workspace (name+dir+file)
  (interactive)
  (+workspace-switch (cl-first name+dir+file) t)
  (delete-other-windows)
  (find-file (concat (cl-second name+dir+file) (caddr name+dir+file)))
  (find-file (concat (cl-second name+dir+file) (cl-second (projectile-recentf-files)))))
